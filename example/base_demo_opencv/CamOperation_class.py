...
class CameraOperation:
    ...
    def __init__(self, obj_cam, st_device_list, n_connect_num=0, b_open_device=False, b_start_grabbing=False,
        ...
        self.window_name = "Camera Display"  # OpenCV窗口名称
        self.latest_image = None  # 最新的图像数据（NumPy数组）
        self.image_lock = threading.Lock()  # 图像数据锁
        ...
        while True:
            ...
            if 0 == ret:
                ...
                if ret == 0:
                    ...
                    image = image_array.reshape((self.st_frame_info.nHeight, self.st_frame_info.nWidth, 3)).copy()

                    # 添加帧信息文本
                    text = f"Frame: {self.st_frame_info.nFrameNum} Size: {self.st_frame_info.nWidth}x{self.st_frame_info.nHeight}"
