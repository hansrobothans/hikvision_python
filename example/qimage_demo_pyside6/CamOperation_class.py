    ...
    def Work_thread(self):
        ...
        while True:
            ...
            if 0 == ret:
                ...
                stConvertParam.pDstBuffer = buf_rgb
                stConvertParam.nDstBufferSize = nRGBSize

                ret = self.obj_cam.MV_CC_ConvertPixelType(stConvertParam)
                if ret == 0:
                    # 创建QImage
                    image = QImage(buf_rgb, self.st_frame_info.nWidth, self.st_frame_info.nHeight,
                                   self.st_frame_info.nWidth * 3, QImage.Format_RGB888)
                    # 复制图像数据，避免buf_rgb被释放
                    image_copy = image.copy()
                    # 发送信号
                    self.image_signal.emit(image_copy)
